...
import { getComprador } from "../graphql/queries";
import { updateComprador } from "../graphql/mutations";
const client = generateClient();
export default function CompradorUpdateForm(props) {
  const {
    id: idProp,
    comprador: compradorModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = compradorRecord
      ? { ...initialValues, ...compradorRecord }
      : initialValues;
    setNombre(cleanValues.Nombre);
    setApellido(cleanValues.Apellido);
    setTipodeidenticiacion(cleanValues.Tipodeidenticiacion);
    setNumerodeidentificacion(cleanValues.Numerodeidentificacion);
    setTelefono(cleanValues.Telefono);
    setEmail(cleanValues.Email);
    setDireccionfiscal(cleanValues.Direccionfiscal);
    setCiudad(cleanValues.Ciudad);
    setEstado(cleanValues.Estado);
    setPais(cleanValues.Pais);
    setErrors({});
  };
  const [compradorRecord, setCompradorRecord] =
    React.useState(compradorModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getComprador.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getComprador
        : compradorModelProp;
      setCompradorRecord(record);
    };
    queryData();
  }, [idProp, compradorModelProp]);
  React.useEffect(resetStateValues, [compradorRecord]);
  const validations = {
    Nombre: [],
    Apellido: [],
    Tipodeidenticiacion: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          Tipodeidenticiacion: Tipodeidenticiacion ?? null,
          Numerodeidentificacion,
        ...
        try {
          ...
          await client.graphql({
            query: updateComprador.replaceAll("__typename", ""),
            variables: {
              input: {
                id: compradorRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "CompradorUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nombre"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || compradorModelProp)}
