...
export default function ApartamentCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setM2(initialValues.m2);
    setValor(initialValues.valor);
    setFracciones(initialValues.fracciones);
    setErrors({});
  };
  const validations = {
    m2: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(new Apartament(modelFields));
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "ApartamentCreateForm")}
      {...rest}
    >
      <TextField
        label="M2"
